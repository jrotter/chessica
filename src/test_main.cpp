...
int VerifyHighEndMoveScore(int ply, int ExpectedScore, long long int TotalMoves, ResetTree *Root, int ExpectedTime)
{
  ...
  printf("Ply=%d, IterativeAB In Place[5]: %ll (savings=%2.3f\%), Score=%d, Time=%d", ply, ABIPMoveCount,((float) (TotalMoves-ABIPMoveCount) * 100.0) / ((float) TotalMoves),ABIPScore,ABIPTime);
  ...
  printf("Ply=%d, IterativeABSaveBest LeaveNodes[5]: %ll (savings=%2.3f\%), Score=%d, Time=%d", ply, ABLNSBMoveCount,((float) (TotalMoves-ABLNSBMoveCount) * 100.0) / ((float) TotalMoves),ABLNSBScore,ABLNSBTime);
  Root->DeleteTree(RUNS_SERIALLY);
...
int Game::VerifyIndividualMoveScore(char SearchRoutineText[], int ExpectedScore, long long int TotalMoves)
{
  ...
  printf("%ll (savings=%2.3f\%), Score=%d, Time=%d", MoveCount[0],((float) (TotalMoves-MoveCount[0]) * 100.0) / ((float) TotalMoves),SearchScore[0],myTime);
...
int VerifyMoveGeneration(int ply, long long int Expected, ResetTree *Root, int ExpectedTime)
{
  ...
  printf("Expected=%ll, Actual=%ll, Score=%d, Time=%d",Expected,SerialMoveCount,SerialScore,SerialTime);
...
int CompareABvsNaive(int ply, ResetTree *Root)
{
  ...
  printf("ABMovesSaved: %ll (%3.0f), ",SerialMoveCount-ABMoveCount,(float) ABMoveCount / ((float) SerialMoveCount));
